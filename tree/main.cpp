...
//функции и методы для горизонтальной печати
struct Node
{
	int data; // Переменная для хранения данных узла
	Node* left, * right; // Указатели на левого и правого потомков узла

	Node(int data) // Конструктор узла, принимающий значение данных
	{
		this->data = data;// Инициализация данных узла
		this->left = this->right = nullptr; // Инициализация указателей на потомков значением nullptr
	}
};

struct HelpPrintHorizont
{
	HelpPrintHorizont* prev; // Указатель на предыдущий элемент HelpPrintHorizont
	string str; // Строка для отображения связи между узлами

	HelpPrintHorizont(HelpPrintHorizont* prev, string str)// Конструктор структуры
	{
		this->prev = prev; // Инициализация указателя на предыдущий элемент
		this->str = str; // Инициализация строки
	}
};
void showHelpPrintHorizont(HelpPrintHorizont* p) // Функция для вывода строк связей
{
	if (p == nullptr) { // Если указатель равен nullptr, выходим из функции
		return;
	}

	showHelpPrintHorizont(p->prev); // Рекурсивный вызов для предыдущего элемента
	cout << p->str; // Вывод строки связи текущего элемента
}

void printTree(Node* root, HelpPrintHorizont* prev, bool isLeft)// Функция для вертикальной печати дерева
{
	if (root == nullptr) { // Если корневой узел равен nullptr, выходим из функции
		return;
	}

	string prev_str = "    "; // Строка для отступа
	HelpPrintHorizont* other = new HelpPrintHorizont(prev, prev_str); // Создание нового элемента

	printTree(root->right, other, true); // Рекурсивный вызов для правого поддерева

	if (!prev) {
		other->str = "---"; // Если это корневой узел, устанавливаем строку связи
	}
	else if (isLeft) // Если текущий узел является левым потомком
	{
		other->str = ".---";// Устанавливаем строку связи для левого потомка
		prev_str = "   |";// Устанавливаем строку отступа для предыдущего элемента
	}
	else { // Если текущий узел является правым потомком
		other->str = "'---"; // Устанавливаем строку связи для правого потомка
		prev->str = prev_str; // Устанавливаем строку отступа для предыдущего элемента
	}

	showHelpPrintHorizont(other);// Вывод строк связей
	cout << " " << root->data << endl; // Вывод значения текущего узла

	if (prev) {
		prev->str = prev_str; // Восстанавливаем строку отступа для предыдущего элемента
	}
	other->str = "   |"; // Устанавливаем строку связи для левого поддерева

	printTree(root->left, other, false); // Рекурсивный вызов для левого поддерева
}
...
int main()
{
	...
	Node* root = new Node(8); // Создание корневого узла
	root->left = new Node(4);  // Создание левого потомка
	root->right = new Node(9); // Создание правого потомка
	root->left->left = new Node(2);// Создание левого потомка левого потомка
	root->left->right = new Node(3); // Создание правого потомка левого потомка
	root->right->left = new Node(10);// Создание левого потомка правого потомка
	root->right->right = new Node(11); // Создание правого потомка правого потомка
	root->left->left->left = new Node(1); // Создание левого потомка левого потомка левого потомка
	printTree(root, nullptr, false); // Вызов функции для горизонтальной печати печати дерева
